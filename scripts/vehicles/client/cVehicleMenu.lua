...
function VehicleMenu:__init()
	...
	self.window:SetSizeRel( Vector2( 0.5, 0.8 ) )
	self.window:SetPositionRel( Vector2( 0.5, 0.5 ) - 
								self.window:GetSizeRel()/2 )
	self.window:SetTitle( "My Vehicles 0 / 0" )
	self.window:SetVisible( self.active )

	self.list = SortedList.Create( self.window )
	self.list:SetDock( GwenPosition.Fill )
	self.list:AddColumn( "Name", )
	self.list:AddColumn( "Health", 32 )
	self.list:AddColumn( "Distance", 64 )
	self.list:AddColumn( "Spawn", 64 )
	self.list:AddColumn( "Waypoint", 64 )
	self.list:AddColumn( "Transfer", 64 )
	self.list:AddColumn( "Remove", 64 )
...
function VehicleMenu:GetActive()
	return self.active
end

function VehicleMenu:SetActive( state )
	self.active = state
	self.window:SetVisible( self.active )
	Mouse:SetVisible( self.active )
end

function VehicleMenu:KeyUp( args )
	if args.key == VirtualKey.F7 then
		self:SetActive( not self:GetActive() )
	end
end

function VehicleMenu:LocalPlayerInput( args )
	if self:GetActive() and Game:GetState() == GUIState.Game then
		return false
	end
end

function VehicleMenu:UpdatePings( list )
...
function VehicleMenu:PressFriendButton(button)
...
function VehicleMenu:UpdateFriendColor(tablerow, steam_id)
...
function VehicleMenu:RemovePlayer( player )
	self.PlayerCount = self.PlayerCount - 1
