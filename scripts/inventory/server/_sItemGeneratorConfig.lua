LootItems = 
{
    -- {Item,Rarity,MinQuantity,MaxQuantity}
    [Lootbox.Types.Level1] = 
    {
        ["AMMO"] = 
        {
            rarity = 0.33,
            items = 
            {
                ["Bubbles"] = {rarity = 0.2, min = 5, max = 10},
                ["Revolver Ammo"] = {rarity = 0.3, min = 4, max = 8},
                ["Handgun Ammo"] = {rarity = 0.3, min = 4, max = 8},
                ["Sawn-Off Ammo"] = {rarity = 0.2, min = 4, max = 8},
            }
        },
        ["EXPLOSIVE"] = 
        {
            rarity = 0.2,
            items = 
            {
                ["Flares"] = {rarity = 0.25, min = 1, max = 1},
                ["Flashbang"] = {rarity = 0.25, min = 1, max = 1},
                ["HE Grenade"] = {rarity = 0.5, min = 1, max = 1},
            }
        },
        ["FOOD & DRINK"] = 
        {
            rarity = 0.2,
            items = 
            {
                ["Chocolate"] = {rarity = 0.08, min = 2, max = 3},
                ["Cookies"] = {rarity = 0.079, min = 1, max = 1},
                ["Peanuts"] = {rarity = 0.08, min = 1, max = 1},
                ["Pretzel"] = {rarity = 0.08, min = 1, max = 1},
                ["Can of Beans"] = {rarity = 0.08, min = 1, max = 1},
                ["Can of Ham"] = {rarity = 0.08, min = 1, max = 1},
                ["Can of Peaches"] = {rarity = 0.07, min = 1, max = 1},
                ["Chips"] = {rarity = 0.07, min = 1, max = 2},
                ["Spicy Chips"] = {rarity = 0.01, min = 1, max = 2},
                ["Macadamia Nuts"] = {rarity = 0.001, min = 1, max = 1},
                ["Coffee"] = {rarity = 0.07, min = 1, max = 2},
                ["Apple Juice"] = {rarity = 0.07, min = 1, max = 2},
                ["Iced Tea"] = {rarity = 0.07, min = 1, max = 2},
                ["Energy Drink"] = {rarity = 0.07, min = 1, max = 2},
                ["Water"] = {rarity = 0.07, min = 1, max = 2},
                ["CamelBak"] = {rarity = 0.02, min = 1, max = 1},
            }
        },
        ["LOCKPICK"] = 
        {
            rarity = 0.08,
            items = 
            {
                ["Lockpick"] = {rarity = 1, min = 1, max = 1},
            }
        },
        ["TOOL"] = 
        {
            rarity = 0.155,
            items = 
            {
                ["Woet"] = {rarity = 0.25, min = 1, max = 1},
                ["BackTrack"] = {rarity = 0.25, min = 1, max = 1},
                ["Vehicle Guard"] = {rarity = 0.25, min = 1, max = 1},
                ["Vehicle Repair"] = {rarity = 0.25, min = 1, max = 1},
            }
        },
        ["WEAPON"] = 
        {
            rarity = 0.035,
            items = 
            {
                ["Bubble Gun"] = {rarity = 0.2, min = 1, max = 1},
                ["Revolver"] = {rarity = 0.3, min = 1, max = 1},
                ["Handgun"] = {rarity = 0.3, min = 1, max = 1},
                ["Sawn-Off Shotgun"] = {rarity = 0.2, min = 1, max = 1},
            }
        },

    },
    [Lootbox.Types.Level2] = 
    {
        ["AMMO"] = 
        {
            rarity = 0.33,
            items = 
            {
                ["Bubbles"] = {rarity = 0.05, min = 10, max = 20},
                ["Revolver Ammo"] = {rarity = 0.2, min = 4, max = 8},
                ["Handgun Ammo"] = {rarity = 0.2, min = 4, max = 8},
                ["Sawn-Off Ammo"] = {rarity = 0.15, min = 4, max = 8},
                ["SMG Ammo"] = {rarity = 0.2, min = 5, max = 10},
                ["Shotgun Ammo"] = {rarity = 0.15, min = 3, max = 6},
                ["Assault Ammo"] = {rarity = 0.05, min = 3, max = 6},
            }
        },
        ["ARMOUR"] = 
        {
            rarity = 0.025,
            items = 
            {
                ["Helmet"] = {rarity = 1, min = 1, max = 1},
            }
        },
        ["BUILD"] = 
        {
            rarity = 0.01,
            items = 
            {
                ["Barrel Stash"] = {rarity = 0.5, min = 1, max = 1},
                ["Garbage Stash"] = {rarity = 0.5, min = 1, max = 1},
            }
        },
        ["EXPLOSIVE"] = 
        {
            rarity = 0.145,
            items = 
            {
                ["Flares"] = {rarity = 0.075, min = 1, max = 1},
                ["Flashbang"] = {rarity = 0.05, min = 1, max = 1},
                ["HE Grenade"] = {rarity = 0.3, min = 1, max = 1},
                ["Smoke Grenade"] = {rarity = 0.075, min = 1, max = 1},
                ["Toxic Grenade"] = {rarity = 0.3, min = 1, max = 1},
                ["Mine"] = {rarity = 0.2, min = 1, max = 1},
            }
        },
        ["GEAR"] = 
        {
            rarity = 0.028,
            items = 
            {
                ["Grapplehook"] = {rarity = 1, min = 1, max = 1},
            }
        },
        ["HEALTH"] = 
        {
            rarity = 0.13,
            items = 
            {
                ["Bandages"] = {rarity = 0.6, min = 1, max = 2},
                ["Healthpack"] = {rarity = 0.4, min = 1, max = 1},
            }
        },
        ["LOCKPICK"] = 
        {
            rarity = 0.08,
            items = 
            {
                ["Lockpick"] = {rarity = 1, min = 1, max = 2},
            }
        },
        ["TOOL"] = 
        {
            rarity = 0.202,
            items = 
            {
                ["BackTrack"] = {rarity = 0.2, min = 1, max = 1},
                ["Vehicle Guard"] = {rarity = 0.2, min = 1, max = 1},
                ["Vehicle Repair"] = {rarity = 0.2, min = 1, max = 1},
                ["Car Paint"] = {rarity = 0.1, min = 1, max = 1},
                ["Battery"] = {rarity = 0.3, min = 1, max = 1},
            }
        },
        ["WEAPON"] = 
        {
            rarity = 0.05,
            items = 
            {
                ["Bubble Gun"] = {rarity = 0.05, min = 1, max = 1},
                ["Revolver"] = {rarity = 0.3, min = 1, max = 1},
                ["Handgun"] = {rarity = 0.25, min = 1, max = 1},
                ["Sawn-Off Shotgun"] = {rarity = 0.2, min = 1, max = 1},
                ["SMG"] = {rarity = 0.2, min = 1, max = 1},
            }
        },

    },
    [Lootbox.Types.Level3] = 
    {
        ["AIRSTRIKE"] = 
        {
            rarity = 0.015,
            items = 
            {
                ["Cruise Missile"] = {rarity = 1, min = 1, max = 1},
            }
        },
        ["AMMO"] = 
        {
            rarity = 0.388,
            items = 
            {
                ["Sawn-Off Ammo"] = {rarity = 0.1, min = 3, max = 6},
                ["SMG Ammo"] = {rarity = 0.2, min = 6, max = 12},
                ["Shotgun Ammo"] = {rarity = 0.2, min = 3, max = 6},
                ["Assault Ammo"] = {rarity = 0.2, min = 5, max = 10},
                ["Sniper Ammo"] = {rarity = 0.1, min = 2, max = 4},
                ["GrLauncher Ammo"] = {rarity = 0.1, min = 1, max = 1},
                ["Machine Gun Ammo"] = {rarity = 0.1, min = 4, max = 8},
            }
        },
        ["ARMOUR"] = 
        {
            rarity = 0.04,
            items = 
            {
                ["Police Helmet"] = {rarity = 0.5, min = 1, max = 1},
                ["Military Helmet"] = {rarity = 0.5, min = 1, max = 1},
            }
        },
        ["BUILD"] = 
        {
            rarity = 0.02,
            items = 
            {
                ["Barrel Stash"] = {rarity = 0.5, min = 1, max = 1},
                ["Garbage Stash"] = {rarity = 0.5, min = 1, max = 1},
            }
        },
        ["EXPLOSIVE"] = 
        {
            rarity = 0.09,
            items = 
            {
                ["Smoke Grenade"] = {rarity = 0.02, min = 1, max = 2},
                ["Toxic Grenade"] = {rarity = 0.25, min = 1, max = 2},
                ["Mine"] = {rarity = 0.25, min = 1, max = 1},
                ["Claymore"] = {rarity = 0.24, min = 1, max = 1},
                ["Molotov"] = {rarity = 0.24, min = 1, max = 1},
            }
        },
        ["GEAR"] = 
        {
            rarity = 0.042,
            items = 
            {
                ["Grapplehook"] = {rarity = 0.1, min = 1, max = 1},
                ["Explosives Detector"] = {rarity = 0.7, min = 1, max = 1},
                ["Combat Backpack"] = {rarity = 0.1, min = 1, max = 1},
                ["Explorer Backpack"] = {rarity = 0.1, min = 1, max = 1},
            }
        },
        ["HEALTH"] = 
        {
            rarity = 0.145,
            items = 
            {
                ["Healthpack"] = {rarity = 1, min = 1, max = 1},
            }
        },
        ["TOOL"] = 
        {
            rarity = 0.17,
            items = 
            {
                ["Vehicle Repair"] = {rarity = 0.18, min = 1, max = 2},
                ["Car Paint"] = {rarity = 0.18, min = 1, max = 1},
                ["Battery"] = {rarity = 0.445, min = 1, max = 2},
                ["Combat Ping"] = {rarity = 0.14, min = 1, max = 1},
                ["Respawner"] = {rarity = 0.055, min = 1, max = 1},
            }
        },
        ["WEAPON"] = 
        {
            rarity = 0.09,
            items = 
            {
                ["Sawn-Off Shotgun"] = {rarity = 0.2, min = 1, max = 1},
                ["SMG"] = {rarity = 0.4, min = 1, max = 1},
                ["Shotgun"] = {rarity = 0.2, min = 1, max = 1},
                ["Assault Rifle"] = {rarity = 0.1, min = 1, max = 1},
                ["Sniper Rifle"] = {rarity = 0.1, min = 1, max = 1},
            }
        },

    },
    [Lootbox.Types.Level4] = 
    {
        ["AIRSTRIKE"] = 
        {
            rarity = 0.03,
            items = 
            {
                ["Cruise Missile"] = {rarity = 0.7, min = 1, max = 2},
                ["Area Bombing"] = {rarity = 0.2, min = 1, max = 1},
                ["Tactical Nuke"] = {rarity = 0.1, min = 1, max = 1},
            }
        },
        ["AMMO"] = 
        {
            rarity = 0.443,
            items = 
            {
                ["Shotgun Ammo"] = {rarity = 0.213, min = 4, max = 8},
                ["Assault Ammo"] = {rarity = 0.21, min = 7, max = 14},
                ["Sniper Ammo"] = {rarity = 0.206, min = 2, max = 4},
                ["GrLauncher Ammo"] = {rarity = 0.12, min = 1, max = 2},
                ["Machine Gun Ammo"] = {rarity = 0.206, min = 5, max = 10},
                ["Rockets"] = {rarity = 0.045, min = 1, max = 2},
            }
        },
        ["ARMOUR"] = 
        {
            rarity = 0.04,
            items = 
            {
                ["Military Vest"] = {rarity = 0.75, min = 1, max = 1},
                ["Kevlar Vest"] = {rarity = 0.25, min = 1, max = 1},
            }
        },
        ["BUILD"] = 
        {
            rarity = 0.042,
            items = 
            {
                ["Proximity Alarm"] = {rarity = 0.6, min = 1, max = 1},
                ["Locked Stash"] = {rarity = 0.4, min = 1, max = 1},
            }
        },
        ["EXPLOSIVE"] = 
        {
            rarity = 0.165,
            items = 
            {
                ["Mine"] = {rarity = 0.338, min = 1, max = 2},
                ["Claymore"] = {rarity = 0.272, min = 1, max = 1},
                ["Molotov"] = {rarity = 0.32, min = 1, max = 1},
                ["C4"] = {rarity = 0.07, min = 1, max = 1},
            }
        },
        ["GEAR"] = 
        {
            rarity = 0.02,
            items = 
            {
                ["Combat Backpack"] = {rarity = 0.308, min = 1, max = 1},
                ["Explorer Backpack"] = {rarity = 0.308, min = 1, max = 1},
                ["Parachute"] = {rarity = 0.192, min = 1, max = 1},
                ["RocketGrapple"] = {rarity = 0.192, min = 1, max = 1},
            }
        },
        ["TOOL"] = 
        {
            rarity = 0.16,
            items = 
            {
                ["Battery"] = {rarity = 0.506, min = 1, max = 2},
                ["Combat Ping"] = {rarity = 0.24, min = 1, max = 1},
                ["Respawner"] = {rarity = 0.124, min = 1, max = 1},
                ["EVAC"] = {rarity = 0.1, min = 1, max = 1},
                ["Hacker"] = {rarity = 0.03, min = 1, max = 1},
            }
        },
        ["WEAPON"] = 
        {
            rarity = 0.1,
            items = 
            {
                ["Shotgun"] = {rarity = 0.125, min = 1, max = 1},
                ["Assault Rifle"] = {rarity = 0.6, min = 1, max = 1},
                ["Sniper Rifle"] = {rarity = 0.1, min = 1, max = 1},
                ["Grenade Launcher"] = {rarity = 0.1, min = 1, max = 1},
                ["Machine Gun"] = {rarity = 0.05, min = 1, max = 1},
                ["Rocket Launcher"] = {rarity = 0.025, min = 1, max = 1},
            }
        },

    },
    [Lootbox.Types.Level5] = 
    {

    },
    [Lootbox.Types.VendingMachineFood] = 
    {
        ["FOOD & DRINK"] = 
        {
            rarity = 1,
            items = 
            {
                ["Chocolate"] = {rarity = 0.125, min = 1, max = 2},
                ["Cookies"] = {rarity = 0.125, min = 1, max = 2},
                ["Peanuts"] = {rarity = 0.125, min = 1, max = 2},
                ["Pretzel"] = {rarity = 0.125, min = 1, max = 2},
                ["Can of Beans"] = {rarity = 0.119, min = 1, max = 2},
                ["Can of Ham"] = {rarity = 0.125, min = 1, max = 2},
                ["Can of Peaches"] = {rarity = 0.125, min = 1, max = 2},
                ["Chips"] = {rarity = 0.125, min = 1, max = 2},
                ["Spicy Chips"] = {rarity = 0.005, min = 1, max = 2},
                ["Macadamia Nuts"] = {rarity = 0.001, min = 1, max = 1},
            }
        },

    },
    [Lootbox.Types.VendingMachineDrink] = 
    {
        ["FOOD & DRINK"] = 
        {
            rarity = 1,
            items = 
            {
                ["Coffee"] = {rarity = 0.2, min = 1, max = 2},
                ["Apple Juice"] = {rarity = 0.2, min = 1, max = 2},
                ["Iced Tea"] = {rarity = 0.2, min = 1, max = 2},
                ["Energy Drink"] = {rarity = 0.2, min = 1, max = 2},
                ["Water"] = {rarity = 0.2, min = 1, max = 2},
            }
        },

    },
}
