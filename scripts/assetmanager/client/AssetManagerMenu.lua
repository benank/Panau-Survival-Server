...
function AssetManagerMenu:UpdateCategoryNames()
    if LocalPlayer:GetValue("MaxVehicles") ~= nil then
        self.categories["Vehicles"].button:SetText(
            string.format(
                "Vehicles (%d/%d)",
                count_table(self.categories["Vehicles"].vehicles),
                LocalPlayer:GetValue("MaxVehicles")
            )
        )
    end

    if LocalPlayer:GetValue("MaxStashes") ~= nil then
        self.categories["Stashes"].button:SetText(
            string.format(
                "Stashes (%d/%d)",
                count_table(self.categories["Stashes"].stashes),
                LocalPlayer:GetValue("MaxStashes")
            )
        )
    end

    if LocalPlayer:GetValue("MaxLandclaims") ~= nil then
        self.categories["Landclaims"].button:SetText(
            string.format(
                "Landclaims (%d/%d)",
                count_table(self.categories["Landclaims"].landclaims),
                LocalPlayer:GetValue("MaxLandclaims")
            )
        )
    end
end
...
function AssetManagerMenu:UpdateVehicleSecondTick()
    for id, vehicle_data in pairs(self.categories["Vehicles"].vehicles) do
        ...
        local health =
            IsValid(vehicle_data.data.vehicle) and vehicle_data.data.vehicle:GetHealth() or vehicle_data.data.health
...
function AssetManagerMenu:PressVehicleButton(btn)
    if self.button_timer:GetSeconds() < 0.5 then
        return
    end
    ...
    elseif type == "Waypoint" then
        Waypoint:SetPosition(
            IsValid(vehicle_data.data.vehicle) and vehicle_data.data.vehicle:GetPosition() or vehicle_data.data.position
        )
